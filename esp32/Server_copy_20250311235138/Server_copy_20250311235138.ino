...
//modbus
ModbusMaster node;
const int sensorIDs[] = {1, 2, 3};   // ID датчиков
...
void read_sensor_data() {
    ...
    // Устанавливаем ID датчика
    node.begin(id, Serial2);
    node.preTransmission(preTransmission);
    node.postTransmission(postTransmission);
    uint8_t result;
    // Читаем скорость ветра
    switch(id){
    case 1:
    result = node.readHoldingRegisters(0x0000, 1);
      if (result == node.ku8MBSuccess) {
      solarRad = node.getResponseBuffer(0);
    } else {
      solarRad = 0;
      Serial.print("Ошибка modbus ID 1 :");
    ...
    result = node.readHoldingRegisters(0x0001, 1);
      if (result == node.ku8MBSuccess) {
      windDir = node.getResponseBuffer(0);
    } else {
      windDir = 0;
      Serial.print("Ошибка modbus ID 2 :");
      Serial.println(result);
    }
      break;
    case 3:
    result = node.readHoldingRegisters(0x0001, 1);
      ...
      windSpeed = 0;
      Serial.print("Ошибка modbus ID 3 :");
      Serial.println(result);
    }
      break;
    }
...
void getData() {
  ...
  add_json_object(106, windDir);
  add_json_object(107, windSpeed);
  add_json_object(108, solarRad);
...
void setup(void) {
  Serial.begin(115200);
  Serial2.begin(9600, SERIAL_8N1, 16, 17); // скорость всех датчиков modbus
  //вопрос
  Wire.begin();
  // Пин управления modbus:
